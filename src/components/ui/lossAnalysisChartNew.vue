...
export default {
    ...
    methods: { 
        ...
        parseUnstructuredData(lossData) {
    ...
    mounted() {
        ...
        // Labels
        let subLabels1 = chart.data.datasets[0].abs_values.map(val => Math.round(val) + ' kWh/m²')
        let subLabels2 = chart.data.datasets[1].abs_values.filter((val, ind) => ind > 4).map(val => Math.round(val) + ' kWh')
        let subLabels3 = chart.data.datasets[2].abs_values.filter((val, ind) => ind > 9).map(val => Math.round(val) + ' kWh')
        let labels2 = [
            ...subLabels1,
            ...subLabels2,
            ...subLabels3,
        ...
        const floatingLabels = {
            ...
            afterDatasetsDraw(chart, args, options) {
                ...
                chart.config.data.abs_values.forEach((val, ind) => {
                    ...
                    let yPosition = model.y - model.height 
