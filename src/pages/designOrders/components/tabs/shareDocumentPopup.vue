...
export default {
  ...
  watch: {
    isShareDocumentPopupVisible: {
      handler(val) {
        console.log("wdwadsadsadas", this.documentContainer);
        if (val) {
          // console.log("@@@calling watcher",this.$route)
          this.fileList = [];
  ...
  methods: {
    ...
    async saveDocuments(fromOrderServicePopups) {
      this.$props.handleSave(this.fileList);
      ...
      //   }
      //   postData.append("uploaded_by", "designer");
