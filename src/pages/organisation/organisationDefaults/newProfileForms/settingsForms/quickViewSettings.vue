        ...
        <VuePerfectScrollbar class="scroll-area">
            ...
            <el-form
                ...
                <el-form-item label="Ridge">
                    ...
                    >
                    <input-length
                        v-if="!profileData.drawing_defaults.quickView.ridgeLocked"
                ...
                <div v-if="isOrgUnirac">
                    ...
                    <el-form-item label="Total module Occupancy">
                        <input
                            v-model.number="moduleOccupancy"
                            disabled="true"
                            name="ModuleOccupancy"
                            type="number"
                            class="inputBoxStyler"
                            autocomplete="off"
                            step="any"
                        />%
                        <p class="formErrors"><span>{{ errors.first('ModuleOccupancy') }}</span></p>
                    </el-form-item>

                    <el-form-item label="Total roof area">
                        <input-length
                            v-model="modelRoofArea"
                            :disabled="true"
                            :name="'RoofArea'"
                            :class-input="'inputBoxStyler'"
                            :convert-value="true"
...
export default {
    ...
    data() {
        return {
            labelPosition: 'left',
            subarrayPanel: {},
            panelInfiniteScrollId: 'fixedMountPanel',
    ...
    computed: {
        ...
        moduleOccupancy() {
            const { moduleLength } = this.profileData.drawing_defaults.subarray.flushMount.moduleProperties;
            const { moduleWidth } = this.profileData.drawing_defaults.subarray.flushMount.moduleProperties;
            const { totalModules } = this.profileData.drawing_defaults.quickView;

            const coverage = parseFloat((moduleLength * moduleWidth * totalModules) / this.modelRoofArea).toFixed(2) * 100;
            return coverage;
        },
        modelRoofArea(){
            console.log(parseFloat(this.roofArea().toFixed(2)))
            return parseFloat(this.roofArea().toFixed(2));
        },
        RidgeValue(){
            if (this.profileData.drawing_defaults.quickView.ridgeLocked) {
                let IBC = 0, IFC = 0;
                if( this.ahjDetails.BuildingCode !== null && this.ahjDetails.BuildingCode !== "NA" ){
                    IBC = parseInt(this.ahjDetails.BuildingCode.slice(0,4));
                }
                if( this.ahjDetails.FireCode !== null && this.ahjDetails.FireCode !== "NA" ){
                    IFC = parseInt(this.ahjDetails.FireCode.slice(0,4));
                }
                if(IBC >= 2018 || IFC >= 2018){
                    if(this.moduleOccupancy > 33){
                        this.ridgeValue = 0.9144;
                    }
                    else{
                        this.ridgeValue = 0.4572;
                    }
                }
                else if( IBC == 0 && IFC == 0 ){
                    this.ridgeValue = 0.4572;
